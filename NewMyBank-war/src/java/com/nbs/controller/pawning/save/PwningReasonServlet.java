 ...
 * @author mmh
 */
public class PwningReasonServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String pwnReasonSin = request.getParameter("pwnReasonEng");
            String pwnReasonEng = request.getParameter("pwnReasonSin");
            BnkPwnPawnReason bnkPwnPawnReason = new BnkPwnPawnReason();
            bnkPwnPawnReason.setCPwnReason(pwnReasonSin);
            bnkPwnPawnReason.setCPwnReasonEng(pwnReasonEng);

            boolean isAdded = bnkPwnPawnMasterDao.savePwnReason(bnkPwnPawnReason);
            ...
            if (isAdded) {
                ...
                for (BnkPwnPawnReason bnkPwnPawnReason1 : bnkPwnPawnReasons) {
                    output += " <tr onclick='setDataIntoTable(this);'><td></td>\n"
                            + "           <td class=\"convertSinhalaIskolaPotha\">" + bnkPwnPawnReason1.getCPwnReason() + "</td>\n"
                            + "           <td class=\"convertSinhalaIskolaPotha\">" + bnkPwnPawnReason1.getCPwnReasonEng() + "</td>\n"
