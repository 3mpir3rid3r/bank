 ...
 * @author mmh
 */
public class LoadMemberComboServlet extends HttpServlet {
...
//            if (request.getSession().getAttribute("BnkRefCustomerCategory") == null) {
            ...
            request.getSession().setAttribute("Possition", bnkCustomerMasterDao.getMemberShipPossition());
            request.getSession().setAttribute("getNewid", bnkCustomerMasterDao.getLAstId());
//            }
            String id = request.getParameter("id");
                
            if(id!=null){
                response.sendRedirect("member/member_details.jsp?i=xx");
            }else{
                response.sendRedirect("member/member_details.jsp");
            }
        } catch (Exception e) {
